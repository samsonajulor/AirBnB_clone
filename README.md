...
```
$ ALX_MYSQL_USER=alx_test ALX_MYSQL_PWD=alx_test_pwd \
ALX_MYSQL_HOST=localhost ALX_MYSQL_DB=alx_test_db ALX_TYPE_STORAGE=db \
[COMMAND HERE]
```
...
during maintenance or to scale the entire project.  The following files are the
setupfiles along with a brief explanation:

* **`dev/setup.sql`:** Drops test and dev databases, and then reinitialize
the database.

  * Usage: `$ cat dev/setup.sql | mysql -uroot -p`

* **`setup_mysql_dev.sql`:** initialize dev database with mysql for testing
...
* **`3-deploy_web_static.py`:** uses 2 functions from (1-pack_web_static.py &
  ...
  * Usage: `$ fab -f 3-deploy_web_static.py deploy -i ~/.ssh/ALX -u ubuntu`
